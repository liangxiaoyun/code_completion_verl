...
def compute_score(data_source, solution_str, ground_truth, extra_info=None):
    score = 0.0
    try:
        if "data_source" in extra_info.keys() and extra_info["data_source"] == "250701-250730_serious_badcase": #4种badcase数据
            badcase_weight = 0.5
            length_weight = 0.5
        ...
        elif "data_source" in extra_info.keys() and extra_info["data_source"] in ["250530_250630_completion_v1_output_empty", "test_python_output_empty_data"]: #输出为空的数据
            score = 1.0 if solution_str == "" else 0.0
            return {"score": score, "empty_reward": score}
        #TODO
        elif "data_source" in extra_info.keys() and extra_info["data_source"] == "250701-250831_sast_error":
            ast_weight = 0.5
            length_weight = 0.5
            score_ast = ast_error_judge(solution_str, ground_truth, extra_info)
            score_length = float(len(solution_str) > 1)
            score = score_ast * ast_weight + score_length * length_weight
            return {"score": score, "ast_error_detection_reward": score_ast, "ast_error_generate_length_reward": score_length}

        else: #高质量均衡采样通用用户挖掘数据
            lev_weight = 0.4
            jaro_weight = 0.3
            length_weight = 0.3
